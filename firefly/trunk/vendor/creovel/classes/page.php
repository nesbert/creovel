...
class page {
	...
	public function set_properties($data, $page = null, $limit = null) {
		...
		// set url path
		$params = get_params();
		$this->url = '/'.$params['controller'].'/'.( $params['action'] ? $params['action'].'/'.( $params['id'] ? $params['id'].'/' : '' ) : '' );

	}
	
	/*
	 * display page limiting option
	 */
	...
	public function display_paging($extra_params = null, $show_page_of = false) {
		...
		if ( $this->total_pages > 1 ) {
			...
			if ( $this->current > 1 ) {
				$str .= '<a class="prev" href="'.$this->url.'?page='.$this->previous.$extra_params.'">&laquo; Prev</a>';
			}
	...
	{
		if ( !$this->total_records ) $this->set_properties($data);
